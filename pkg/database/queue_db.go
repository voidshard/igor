...
func (q *defaultQDB) SetRunsState(in []*structs.Run, st structs.Status, msg string) error {
	if in == nil || len(in) == 0 {
		return nil
	}
	if !structs.IsFinalStatus(st) {
		return fmt.Errorf("%w %s is not a final status (errored, completed, skipped)", errors.ErrInvalidState, st)
	}
	ids := []*IDTag{}
	for _, t := range in {
		ids = append(ids, &IDTag{ID: t.ID, ETag: t.ETag})
	}
	msgs := []string{}
	if msg != "" {
		msgs = []string{msg}
...
// This func is restricted to setting a final state (errored, completed, skipped).
func (q *defaultQDB) SetTasksState(in []*structs.Task, st structs.Status) error {
	if in == nil || len(in) == 0 {
		return nil
	}
	if !structs.IsFinalStatus(st) {
...
func (q *defaultQDB) Runs(id []string) ([]*structs.Run, error) {
